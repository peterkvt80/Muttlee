...
class MAPCHAR {
   ...
   */
  useSecondG0 (value) {
    this.second = (value === 0 || value === false)? 0 : 1
  }
  ...
  map (ch) {
    ...
    const lang = this.language[this.second]
    const region = this.region[this.second]
    switch (region) {
      case 0 : // West Europe
        switch (lang) {
  ...
  MapGreek (ch) { // 6:7
    switch (ch) {
      ...
      default:
        ...
        if ((ch >= '@') && (ch <= '~')) {
          const p1 = ch.charCodeAt(0)
          const p2 = 0x390
          const p3 = '@'.charCodeAt(0)
          const p4 = p1 + p2 - p3
          const ch2 = char(p4)
          return ch2
        }
