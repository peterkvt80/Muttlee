...
ServiceList=function(callback) // This callback is used to send a page
{
  ...
  var targetPage=new TextPage(data);
    ...
    */
	findService=function(services,name)
	{
	...
	*/
	this.doLoad=function(pageData)
	{
		...
		filename=data.S+data.p+'.ttix';	// This is the actual filename that we want to load	
...
targetPage.pageNumber=data.p;// having trouble setting this and making it stick				
				// console.log("[read] row=");console.log(data);
			}.bind(this)			
			); // Allow this to be accessed inside doLoad 
		...
		// console.log('[ServiceList::doLoad] called '+filename+' data.x='+data.x);
